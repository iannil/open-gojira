...
- **L5 `recover_stale_runs` 10min 阈值 vs long-running pipeline race (2026-06-18 发现, 2026-06-19 修)**: 已修 — `recover_stale_runs` 阈值从 10min 提到 **6h** (`manager.py:311`)。原 10min 设计为 server-crash 检测,但 backfill (5626 股 × N 年 ≈ 2-3h) 被误标 FAILED。修后 sweep 只清真正 stuck 6h+ 的 pipeline,backfill 期间 status 保持 RUNNING 直到完成。
- **L6 dead_letter 不自动 resolve on pipeline 重跑成功 (2026-06-19 发现)**: `manager._execute_with_db` 跑完 pipeline 后**只 push 新 dead_letter**(失败 stock),不会 resolve 旧的(成功 stock)。即使 pipeline 重跑成功 (dividends 6-18 +38159 行 / financials 6-17 +23794 行已写入),旧 dead_letter 永远 pending。证据: 6-17 16:13 circuit open 失败的 10708 条 dead_letter,直到 6-19 01:30 手动 SQL UPDATE 才 resolve。修复路径: pipeline 成功 stock 应 batch resolve 对应 dead_letter。手动清理: `UPDATE dead_letter_records SET status='resolved' WHERE status='pending' AND created_at < '{cutoff}'`
- **L7 api_usage_logs 表 0 行 (2026-06-19 发现)**: `lixinger_client.py` 未写 `api_usage_logs` 表 (instrumentation 缺失)。DataManagement 页面"API 用量(今日)调用次数"始终显示 0,与实际不符 (6-19 backfill 跑了 5025+ 调用)。**不影响生产链路** (plan_runner / freshness gate 不读此表)。仅影响用户看不到 API 用量。修复路径: 在 lixinger_client 调用后写一行 api_usage_logs (约 30 分钟)
- **L8 财报 pipeline 默认 granularity=y (2026-06-19 发现)**: `financial_pipeline.extract` 默认年报,API 层 `/pipeline/financials/start` 不读 granularity 参数。已修三层透传 (commit `e164307` + `c6bc196` Pydantic 校验)。全量 backfill (`a6a3a66a` 5626 股 × 9 季) 因 L9 中断,只完成 25% (1386/5626 股),2026-Q1 季报 0 覆盖。待重跑。
- **L9 backend `--reload` 中断 daemon thread (2026-06-19 发现)**: `dev.sh` 用 `uvicorn --reload`,修改 `app/` 下任意 .py 触发重启,正在跑的后台 pipeline daemon thread 死亡,`pipeline_runs.status` 一直 `running` 无人写回。证据: 6-19 financials 季报 backfill `a6a3a66a` 在我连续 commit (`e164307` granularity + `c6bc196` Pydantic + `49d1b94` README) 期间中断。**不阻塞 plan_runner** (freshness gate 只看 data_freshness 真实状态)。修复路径: (a) 跑 long-running pipeline 用 `background=False` 同步模式 (b) `dev.sh` 加 `--reload-dir` 限定监听范围 (c) 生产模式去 `--reload`
- **L10 SQLite `datetime('now')` 返回 UTC 跟北京 DB 字段不一致 (2026-06-19 时区改造后发现)**: SQLite 内置 `datetime('now')` / `CURRENT_TIMESTAMP` / SQLAlchemy `func.now()` 全部返回 UTC。但项目 DB timestamp 字段是 naive 北京。任何 raw SQL 查询用 `datetime('now')` 跟 DB 字段比较会差 8 小时。**代码层 0 处残留**(全用 Python `now()` 写入),**只影响手动 raw SQL 查询**(运维 / AI / 用户)。**已加 helper**: `from app.core.datetime_utils import beijing_now_sql, beijing_now_minus_sql` → SQL 表达式 `datetime('now', '+8 hours')` 等。约定: 手动查 SQL 时用 `datetime('now', '+8 hours')` 替代 `datetime('now')`。
