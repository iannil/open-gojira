...
1. **跑测试确认基线**: `cd backend && .venv/bin/python -m pytest tests/ --tb=no -q` → 应 1181 passed
2. **看 git log**: `git log --oneline -20` → 应看到本会话 11 个 commits
3. **看 DB 状态**: `sqlite3 backend/data/gojira.db "SELECT 'drafts', COUNT(*) FROM drafts UNION ALL SELECT 'candidates', COUNT(*) FROM candidates WHERE status='active' UNION ALL SELECT 'holdings', COUNT(*) FROM holdings WHERE sell_date IS NULL"` → 应 10 drafts / 12 candidates / 0 holdings
...
- 死代码模式重复出现 (F4/F15/daily_industry_sync) — 加新公共方法必须 ≥1 调用方
