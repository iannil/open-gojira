...
**前端 (6/6)**: StockDetail 三态卡片 (截图 02) ✅ / Edit modal proposed 态 (截图 03) ✅ / Edit modal active 态 PATCH (截图 04) ✅ / Cockpit badge proposed 计数 + 30s 刷新 (截图 01) ✅ / **Cockpit badge 失败态红色** ✅ — mock propose_for_run 抛 ZhipuClientError 写 audit_log → API last_proposal.status="failed" → 浏览器 badge "上次 propose 失败 (run 99)" + 错误详情 (截图 05 验证)
