...
6. [理解 Autopilot（自动调度）](#6-理解-autopilot自动调度)
7. [故障排查](#7-故障排查)
...
| 数据库 | SQLite + WAL 模式（单文件，不需要 Docker / PostgreSQL / Redis） |
| 数据源 | [Lixinger 理杏仁](https://www.lixinger.com) — 唯一 A 股数据源 |
| LLM | 智谱 GLM（默认 glm-4.7） — 用于深度研究与智能筛选 |
| 调度 | APScheduler + Asia/Shanghai — 工作日自动运行 |

### 1.4 设计原则

1. **除真实券商下单外全自动化** — 筛选 / 研究 / 草稿 / 审计 / 告警都自动
2. **架构极简** — 不引入 PostgreSQL / Redis / Docker / CI 等基础设施
...
打开 **http://localhost:3000/data-management**，确认数据是否已同步。如果还没同步，跳到 [第 3 章](#3-首次数据同步)。
...
打开 **http://localhost:3000/data-management**，在「Pipeline 控制」Tab 中：
1. 选择 pipeline 类型
2. 点「启动」
3. 在「运行历史」看进度
...
Gojira 在后台有一组精心调度的定时任务，周一到周五自动运行。所有任务使用 Asia/Shanghai 时区，自动识别 A 股交易日。

### 6.1 日常数据同步

| 频次 | 任务 | 说明 |
|---|---|---|
| 每交易日 09:00 | 交易日历检查 | 确认今天是否开市 |
| 每交易日 10:00 | 估值更新 | 拉取最新 PE/PB/DYR |
| 每交易日 10:30 | 财报增量同步 | 检查是否有新财报发布 |
| 每日 12:00 | K线增量同步 | 补全最近交易日 K 线 |

### 6.2 智能筛选任务

| 频次 | 任务 | 说明 |
|---|---|---|
| 周五 14:00 | 质量筛选 | LLM 对全市场做第一道质量过滤 |
| 周五 15:00 | 深度研究 | 对通过质量筛选的股票进行四位大师深度分析 |
| 周六 10:00 | 产业链扫描 | 扫描产业链卡点变化 |
| 周一 09:00 | 论点评分 | 跟踪持仓论点，检查是否被证伪 |

> 所有任务均可在 **http://localhost:3000/scheduler** 页面手动触发或暂停。
...
```bash
./dev.sh stop
lsof -ti :3000 -ti :3001 | xargs kill -9  # 强制清理端口
./dev.sh
```
...
说明数据同步失败。检查：
- Lixinger token 是否有效（`python spikes/lixinger_token_verification.py`）
- 网络是否可达
- 到 **http://localhost:3000/data-management** 手动触发同步
...
| 文档 | 说明 |
|---|---|
| [`docs/progress/STATUS.md`](docs/progress/STATUS.md) | 项目当前状态快照 |
| [`docs/active/redesign-decisions-v2.md`](docs/active/redesign-decisions-v2.md) | v2 架构设计决策（20 项） |
...
🟢 **v2 全链路闭环** · 26 路由 · 19 前端页面 · 30 ORM 模型 · 686 测试通过 · Alembic head: `v2_4_drop_holdings_table`
